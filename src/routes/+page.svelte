<script lang='ts'>

</script>

<!-- Mobile -->
<div class="sm:hidden max-h-[95%] max-w-[90%] grid grid-rows-3 py-4
            text-gray-100 antialiased font-bubbler">
    <div class="flex justify-center relative top-4">
        <img src="https://i.imgur.com/Uo4nGZx.png" alt="img" class="animate-bounce w-[55vw] h-[55vw]">
    </div>
    <div class="flex flex-col gap-2 text-2xl relative top-4 px-4">
        <div>
            <span class="font-semibold">Hello,</span>
        </div>
        <div class="pb-8">
            <span>I'm an adaptable and driven computer science student with diverse skills in software engineering, full-stack development, and emerging technologies. Fluent in English, Spanish, and Portuguese.</span>
        </div>
        <div class="flex gap-5 justify-center">
            <a href="https://github.com/mtzsouza" target="_blank" rel="noreferrer noopener">
                <i class="fa-brands fa-github fa-xl" style="color: #ffffff;"></i>
            </a>
            <a href="https://linkedin.com/in/mtzsouza/" target="_blank" rel="noreferrer noopener">
                <i class="fa-brands fa-linkedin fa-xl" style="color: #ffffff;"></i>
            </a>
            <a href="https://drive.google.com/file/d/1YGMe66hTptEfjz2HSwR4DJuTVl0Zu23R/view?usp=sharing" target="_blank" rel="noreferrer noopener">
                <i class="fa-solid fa-file-lines fa-xl" style="color: #ffffff;"></i>
            </a>
        </div>
    </div>
</div>

<!-- Desktop -->
<div class="hidden sm:flex max-h-[95%] max-w-[90%] grid grid-flow-col grid-cols-2 justify-around overflow-hidden
            text-gray-100 antialiased font-bubbler">
    <div class="flex flex-col gap-6 max-w-[50%] tracking-widest">
        <div class="text-4xl pt-12">
            <span class="font-semibold">Hello!</span>
        </div>
        <div class="text-3xl">
            <span>I'm an adaptable and driven computer science student with diverse skills in software engineering, full-stack development, and emerging technologies. Fluent in English, Spanish, and Portuguese.</span>
        </div>
        <div class="flex gap-5 justify-center text-3xl">
            <a href="https://github.com/mtzsouza" target="_blank" rel="noreferrer noopener">
                <i class="fa-brands fa-github fa-xl" style="color: #ffffff;"></i>
            </a>
            <a href="https://linkedin.com/in/mtzsouza/" target="_blank" rel="noreferrer noopener">
                <i class="fa-brands fa-linkedin fa-xl" style="color: #ffffff;"></i>
            </a>
            <a href="https://drive.google.com/file/d/1YGMe66hTptEfjz2HSwR4DJuTVl0Zu23R/view?usp=sharing" target="_blank" rel="noreferrer noopener">
                <i class="fa-solid fa-file-lines fa-xl" style="color: #ffffff;"></i>
            </a>
        </div>
    </div>
    <div class="flex justify-center max-w-[30%] relative bottom-10">
        <img src="https://i.imgur.com/Uo4nGZx.png" alt="img" class="object-cover overflow-visible animate-bounce">
    </div>
</div>